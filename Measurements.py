...
class Measurements:
    ...
    def _preload(self) -> None:
        for item in os.scandir(self.csv_dir):
           if self._validate_path(item.path):
               features: tuple[str, str, str] | None = self._get_features_measure_name(item.name)
               if features is None:
                   continue
               self._add_preTimeStamps(item.path)

    def _add_preTimeStamps(self,path:str) -> None:
        reader: csv.reader = csv.reader(open(path,"r",encoding="utf-8"))
        data: list[list[str]] = []
        ...
        for x in range(1, len(data[0])):
            Kod: str = data[1][x]  # 1
            Wskaznik: str = data[2][x]  # 2
            Czestotliwosc: str = data[3][x]  # 3  # czas usredniania
            Jednostka: str = data[4][x]
            t: TimeSeries = TimeSeries(Wskaznik,Kod,Czestotliwosc,[],[],Jednostka)
        ...
        for lst in self._dTimeSeries.values():
            for t in lst:
                if isinstance(t,tuple):
                    timeSerie: TimeSeries = t[0]
                else:
                    timeSerie: tuple[TimeSeries, str, int] = t
                if timeSerie.quantity == parameter_name:
                    return True
        return False

    def _full_load_serie(self,code:str) -> list[TimeSeries]:
        """Load all TimeSerie from dict[code]"""
        if code not in self._dTimeSeries:
            raise ValueError(f"code is not in self._dTimeSeries")
        new_list: list[TimeSeries | tuple[TimeSeries, str, int]] = []
    ...
    def detect_all_anomalies(self,validators:list[SeriesValidator], preload: bool = False)->dict:
        ...
        if not preload:
            ...
            for code, items in self._dTimeSeries.items():
                for item in items:
                    for v in validators:
                        a: str = self._analyze(v,item)#v.analyze(item)
                        if a != '':
                            if not code in result:
                                result[code] = [a]
                            else:
    ...
    def _get_features_measure_name(self, csv_name:str)->tuple[str, str, str] | None:
...
if __name__ == "__main__":
    path: str = "test_5"
    x: Measurements = Measurements(path)
    print(len(x))
    ...
    print()
    print("preloaded:")
    preload:dict = x.detect_all_anomalies([OutlierDetector(1),ZeroSpikeDetector(),TresholdDetector(0.1)],True)
